...
from timeVAE.data_utils import (
    load_yaml_file,
    load_data,
    split_data,
    scale_data,
...
from timeVAE.vae.vae_utils import (
...
from timeVAE.visualize import plot_samples, plot_latent_space_samples, visualize_and_save_tsne


def run_vae_pipeline(dataset_name: str, vae_type: str, max_epochs: int):
#def run_vae_pipeline(dataset_name: str, vae_type: str):
    # ----------------------------------------------------------------------------------
    # Load data, perform train/valid split, scale data

    # read data
    data = load_data(data_dir=timeVAE.paths.DATASETS_DIR, dataset=dataset_name)
    ...
    hyperparameters = load_yaml_file(timeVAE.paths.HYPERPARAMETERS_FILE_PATH)[vae_type]
    ...
    model_save_dir = os.path.join(timeVAE.paths.MODELS_DIR, dataset_name)
    # save scaler
    ...
    visualize_and_save_tsne(
        ...
        save_dir=os.path.join(timeVAE.paths.TSNE_DIR, dataset_name),
        max_samples=2000,
    )
    ...
    save_data(
        ...
        output_file=os.path.join(
            os.path.join(timeVAE.paths.GEN_DATA_DIR, dataset_name),
